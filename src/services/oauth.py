...
class OAuthService:
    ...
    async def _run_callback_server(self, expected_state: str) -> asyncio.Future:
        ...
        async def handle_callback(request):
            ...
            if not code:
                ...
                return web.Response(text=f"Authorization failed: {error} - {error_desc}", status=400)
        ...
        print(f"Callback server listening on http://localhost:{self.settings.oauth_server_port}/oauth/callback")
    ...
    async def get_location_token(self, location_id: str, force_refresh: bool = False) -> str:
        ...
        if response.status_code not in (200, 201):
            raise Exception(f"Failed to get location token: {response.status_code} - {response.text}")
